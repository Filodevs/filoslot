import { AppointmentStatus } from '../appointment';
import { IResource } from '../resource';

export const RESOURCE_MOCK: IResource[] = [
  {
    id: 'r1',
    name: 'Jorge Beltrán',
    role: 'Barbero Senior',
    appointments: [
      {
        id: 'a1',
        clientName: 'Juan Pérez',
        clientPhone: '',
        startTime: new Date('2024-06-01T10:00:00'),
        endTime: new Date('2024-06-01T10:30:00'),
        duration: 30,
        resourceId: 'r1',
        serviceId: 's1',
        status: AppointmentStatus.pending,
      },
      {
        id: 'a2',
        clientName: 'María Gómez',
        clientPhone: '',
        startTime: new Date('2024-06-01T11:00:00'),
        endTime: new Date('2024-06-01T11:45:00'),
        duration: 45,
        resourceId: 'r1',
        serviceId: 's2',
        status: AppointmentStatus.completed,
      },
    ],
  },
  { id: 'r2', name: 'Carlos M.', role: 'Estilista', appointments: [] },
  {
    id: 'r3',
    name: 'Andrés P.',
    role: 'Barbero Junior',
    appointments: [
      {
        id: 'a3',
        clientName: 'Lucía Fernández',
        clientPhone: '',
        startTime: new Date('2024-06-01T12:00:00'),
        endTime: new Date('2024-06-01T13:00:00'),
        duration: 60,
        resourceId: 'r3',
        serviceId: 's3',
        status: AppointmentStatus.canceled,
      },
      {
        id: 'a4',
        clientName: 'Miguel Torres',
        clientPhone: '',
        startTime: new Date('2024-06-01T14:00:00'),
        endTime: new Date('2024-06-01T14:30:00'),
        duration: 30,
        resourceId: 'r3',
        serviceId: 's1',
        status: AppointmentStatus.pending,
      },
    ],
  },
];
